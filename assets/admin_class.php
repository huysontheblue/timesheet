...
Class Action {
	...
	function save_manager(){
		...
		if(!empty($password))
			$data .= ", password='".md5($password)."' ";
		$check = $this->db->query("SELECT * FROM managers where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO managers set $data");
		}else{
			$save = $this->db->query("UPDATE managers set $data where id = $id");
		}
	...
	function delete_manager(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM managers where id = ".$id);
		if($delete){
			return 1;
		}
	}
